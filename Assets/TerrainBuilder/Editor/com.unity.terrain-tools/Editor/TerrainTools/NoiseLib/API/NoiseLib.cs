    ...
    internal static partial class NoiseLib
    {
        ...
        private static string[] LoadNoiseSource(INoiseType[] noiseTypes)
        {
            ...
            foreach (INoiseType noise in noiseTypes)
            {
                ...
                if (!File.Exists(path))
                {
                    Debug.LogError("NoiseLib: Noise Include File could not be found! Skipping generation of: " + desc.name + " at " + path);  
                }
                else
                {
        ...
        public static void GenerateShaders()
        {
            ...
            foreach(KeyValuePair<Type, INoiseShaderGenerator> pair in generators)
            {
                ...
                {
                    Debug.LogError("Could not find specified template file for noise shader generator: " +  generator + " at " + generatorDesc.templatePath);
                    continue;
                } 
